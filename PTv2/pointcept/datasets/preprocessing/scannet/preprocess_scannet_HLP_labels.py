...
def handle_process(
    scene_path, output_path, labels_pd, train_scenes, val_scenes, hlp_root, parse_normals=True
):
    ...
    if split_name != "test":
        ...
        hlp_path = os.path.join(hlp_root, f"{scene_id}_predicted_labels.npy")

        semantic_HLP20 = np.load(hlp_path)
        # print("hlp_path: ", hlp_path)
        semantic_HLP20 = semantic_HLP20 - 1
        save_dict["segment20"] = semantic_HLP20
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hlp_root",
        required=True,
        help="Path to the folder containing HLP predicted label npy files.",
    )
