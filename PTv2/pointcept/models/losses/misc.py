...
class CrossEntropyLoss(nn.Module):
    ...
    def forward(self, pred, target_dict):
        if isinstance(target_dict, dict) and "labeled_mask" in target_dict:
            labeled_mask = target_dict["labeled_mask"]
        else:
            labeled_mask = None 
        ...
        if labeled_mask is None:
            ...
            per_point_loss = F.cross_entropy(
                pred,
                target,
                weight=self.weight,
                reduction='none',
            ...
            alpha = self.pseudo_loss_weight
            pseudo_mask = (~labeled_mask) & (target != -1)

            device = pred.device
            dtype = pred.dtype

            if labeled_mask.any():
                loss_selected = per_point_loss[labeled_mask].mean()
            else:
                loss_selected = torch.tensor(0.0, device=device, dtype=dtype, requires_grad=True)

            if pseudo_mask.any():
                loss_pseudo = per_point_loss[pseudo_mask].mean()
            else:
                loss_pseudo = torch.tensor(0.0, device=device, dtype=dtype, requires_grad=True)
            loss = loss_selected + alpha * loss_pseudo
            return loss
