...
class HyperGraph():
    ...
    def LabelPropagation(self, Y):
        edge_degrees = cp_sparse.csr_matrix(cp.sum(self.sparse_H, axis=0))
        ...
        sparse_H_transpose = self.sparse_H.transpose()

        vertex_degrees = cp_sparse.csr_matrix(cp.sum(self.sparse_H, axis=1))
        Dv_inv_sqrt = diags((1 / vertex_degrees.A.ravel()) ** (0.5))
        Dv_inv_sqrt_sparse = cp_sparse.csc_matrix(Dv_inv_sqrt)

        DvH = Dv_inv_sqrt_sparse.dot(self.sparse_H)
        HDv = DvH.transpose()
        DvHDe = DvH.dot(De_inv)
        DvHDeHDv = DvHDe.dot(HDv)

        Y_hat = cp.linalg.inv(cp.eye(DvHDeHDv.shape[0]) - 0.5*DvHDeHDv).dot(Y)
        return Y_hat
