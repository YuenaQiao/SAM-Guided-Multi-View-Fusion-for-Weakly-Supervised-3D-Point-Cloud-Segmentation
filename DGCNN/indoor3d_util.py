...
SAM_DATA_PATH = os.path.join(ROOT_DIR, 'dataset', 'SAM_data')
...
def collect_point_label(anno_path, out_filename, file_format='txt', s3dis_data_path=DATA_PATH, hlp_data_path=HLP_DATA_PATH):
    ...
    area = os.path.basename(os.path.dirname(os.path.dirname(anno_path)))
    room = os.path.basename(os.path.dirname(anno_path))


    s3dis_room_pcd_txt = os.path.join(s3dis_data_path, area, room, f"{room}.txt")
    points = np.loadtxt(s3dis_room_pcd_txt, usecols=(0, 1, 2, 3, 4, 5))
    data = points
    

    # hlp labels 
    HLP_labels_path = os.path.join(hlp_data_path,f"{area}/{area}_{room}_predicted_labels.txt")

    # print("HLP_labels_path: ", HLP_labels_path)
    # Load new labels
    new_labels = np.loadtxt(HLP_labels_path)

    if new_labels.shape[0] != data.shape[0]:
        raise ValueError("The pseudo labels generated by HLP do not match the total number of points in this scene.")

    # Append new labels to data
    data_label = np.concatenate([data, new_labels[:, np.newaxis]], axis=1)  # Nx7
...
def room2blocks_wrapper_normalized(data_label_filename, num_point, block_size=1.0, stride=1.0,
    ...
    if data_label_filename[-3:] == 'txt':
        ...
        label = data_label[:, -1]
        # print(label)
        non_ignore_indices = np.where(label != -1)[0]
